        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4">
          {filtered.map(item => {
            ...
            return (
              ...
              >
                {/* Thumbnail — uses SmartImage with YouTube fallback */}
                <div className="relative aspect-video w-full overflow-hidden">
                  <SmartImage
                    src={thumbUrl}
                    alt={item.title}
                    fallbackGradient={colors.gradient}
                    fallbackContent={<Icon className="h-10 w-10 sm:h-12 sm:w-12 opacity-30 text-white" />}
